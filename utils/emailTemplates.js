...
function verifyEmailTemplate({ token }) {
  const link = `${cfg.frontendURL}/auth/verify-email?token=${encodeURIComponent(
    token
  )}`;
  return {
  ...
  return {
    ...
    html: `
      ...
      <ul>
        ...
        <li><strong>Amount:</strong> ${purchase.amount} ${
      purchase.currency
    }</li>
      </ul>
  ...
  return {
    ...
    html: `
      ...
      <ul>
        ...
        ${
          event.locationType
            ? `<li><strong>Location:</strong> ${event.locationType} — ${
...
function calendarStatusEmail({ event, status, reason, note }) {
  const extra =
    reason || note ? `<p><strong>Note:</strong> ${reason || note}</p>` : "";
  return {
...
function feedbackRequestEmail({ session, forRole, baseUrl }) {
  const link = `${baseUrl}/feedback?sessionId=${encodeURIComponent(
    session.id
  )}&role=${encodeURIComponent(forRole)}`;
  ...
  return {
    ...
    html: `
      ...
      <ul>
        ...
        <li><strong>Started:</strong> ${new Date(
          session.startedAt
        ).toLocaleString()}</li>
        ${
          session.endedAt
            ? `<li><strong>Ended:</strong> ${new Date(
                session.endedAt
              ).toLocaleString()}</li>`
            : ""
        }
